...
class ShoppingRepository {
  ...
  Stream<List<ShoppingItem>> watchItems(String listId) {
    final query = _db.select(_db.shoppingListItems)
      ..where((i) => i.shoppingListId.equals(listId) & i.deletedAt.isNull())
  ...
  Future<void> archiveList(String listId, {required bool archived}) async {
    ...
    await (_db.update(
      _db.shoppingLists,
    )..where((l) => l.id.equals(listId))).write(
  ...
  Future<void> softDeleteList(String listId) async {
    ...
    await _db.transaction(() async {
      await (_db.update(
        ...
        db.ShoppingListsCompanion(deletedAt: Value(now), updatedAt: Value(now)),
      );
      await (_db.update(
        _db.shoppingListItems,
      )..where((i) => i.shoppingListId.equals(listId))).write(
        db.ShoppingListItemsCompanion(
          deletedAt: Value(now),
          updatedAt: Value(now),
        ),
      );
